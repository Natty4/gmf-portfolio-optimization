...
from src.models import fit_arima_on_log_returns, arima_predict_prices, fit_lstm, lstm_recursive_forecast, calculate_metrics
...
from src.visualization import create_interactive_dashboard, enhanced_risk_analysis, generate_all_visualizations
...
def main():
    ...
    log_ret_train, log_ret_test = chronological_split(log_ret[["TSLA"]], TRAIN_END, TEST_END)
    ...
    arima_price_series = pd.Series(arima_fc_price_test, index=adj_test.index[:steps_test], name="ARIMA_FC_Price")

    # 5) LSTM
    print("Fitting LSTM model...")
    lstm_model, scaler = fit_lstm(log_ret_train['TSLA'], lookback=LSTM_LOOKBACK, seed=SEED)
    lstm_fc_logret_test = lstm_recursive_forecast(lstm_model, scaler, log_ret_train['TSLA'], steps_test, LSTM_LOOKBACK)
    lstm_price_test = reconstruct_prices_from_log_returns(adj_train['TSLA'].iloc[-1], lstm_fc_logret_test)
    lstm_price_series = pd.Series(lstm_price_test, index=adj_test.index[:steps_test], name="LSTM_FC_Price")
    ...
    if winner == "ARIMA":
        ...
        fc_price_future = reconstruct_prices_from_log_returns(last_price, fc_logret_future)
        try:
            ...
            lower_log, upper_log = residual_bootstrap_intervals(log_ret['TSLA'], fc_logret_future)
        ...
        fc_logret_future = lstm_recursive_forecast(lstm_model, scaler, log_ret['TSLA'], steps_future, LSTM_LOOKBACK)
        fc_price_future = reconstruct_prices_from_log_returns(last_price, fc_logret_future)
        lower_log, upper_log = residual_bootstrap_intervals(log_ret['TSLA'], fc_logret_future)
        lower = reconstruct_prices_from_log_returns(last_price, lower_log)
        upper = reconstruct_prices_from_log_returns(last_price, upper_log)
    ...
    future_idx = pd.bdate_range(start=adj.index[-1] + pd.offsets.BDay(1), periods=steps_future)
    fc_df = pd.DataFrame({"Price_FC": fc_price_future, "Lower": lower, "Upper": upper}, index=future_idx)
    ...
    plt.fill_between(fc_df.index, fc_df['Lower'], fc_df['Upper'], alpha=0.2, label="Confidence Band")
    ...
    tsla_forecast_daily = pct_ret['TSLA'].mean() if fc_daily_logret.isna().any() else float(fc_daily_logret.mean())
    tsla_historical_daily = pct_ret['TSLA'].mean()
    tsla_exp_daily = 0.5 * tsla_forecast_daily + 0.5 * tsla_historical_daily
    tsla_exp_daily = pct_ret['TSLA'].mean() if np.isnan(tsla_exp_daily) else tsla_exp_daily
    tsla_exp_ann = annualize_return(tsla_exp_daily)

    hist_daily = adj.pct_change().dropna()
    mu_hist_daily = hist_daily.mean()
    bnd_exp_ann = annualize_return(mu_hist_daily['BND']) if not np.isnan(mu_hist_daily['BND']) else 0.02
    spy_exp_ann = annualize_return(mu_hist_daily['SPY']) if not np.isnan(mu_hist_daily['SPY']) else 0.08
    ...
    bt_start = "2024-08-01"; bt_end = "2025-07-31"
    ...
    try:
        report_path = generate_executive_summary(
            arima_metrics, lstm_metrics, winner, portfolio_weights, backtest_results, ARTIFACTS_DIR
        )
        ...
        print(f"LSTM Improvement: {((1-lstm_metrics['RMSE']/arima_metrics['RMSE'])*100):.1f}%")
        print(f"Strategy Return: {strat_metrics['total_return']*100:.1f}%")
        print(f"Benchmark Return: {bench_metrics['total_return']*100:.1f}%")
        print(f"Outperformance: {(strat_metrics['total_return']-bench_metrics['total_return'])*100:+.1f}%")
    ...
    print(f"\n🎯 KEY TAKEAWAYS:")
    print(f"   • LSTM improved accuracy by {((1-lstm_metrics['RMSE']/arima_metrics['RMSE'])*100):.1f}%")
    print(f"   • Strategy outperformed benchmark by {(strat_metrics['total_return']-bench_metrics['total_return'])*100:+.1f}%")
    print(f"   • Max Sharpe portfolio: {portfolio_weights['max_sharpe']['TSLA']*100:.1f}% TSLA")
