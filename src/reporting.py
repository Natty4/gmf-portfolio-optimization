...
def generate_executive_summary(
    ...
    model_data = [
        ...
        [
            ...
            f"{arima_metrics['MAPE_%']:.1f}",
        ],
        [
            "LSTM",
            f"{lstm_metrics['MAE']:.2f}",
            f"{lstm_metrics['RMSE']:.2f}",
            f"{lstm_metrics['MAPE_%']:.1f}",
        ],
        [
            "Improvement",
            f"{(1-lstm_metrics['MAE']/arima_metrics['MAE'])*100:.1f}%",
    ...
    pdf.cell(
        0,
        8,
        f"Selected Model: {winner} (Lower values indicate better performance)",
        0,
        1,
    )
    ...
    perf_data = [
        ...
        [
            "Total Return",
            f"{strat['total_return']*100:.1f}%",
            f"{bench['total_return']*100:.1f}%",
            f"{outperformance*100:+.1f}%",
        ],
        [
            "Annualized Return",
            f"{strat['ann_return']*100:.1f}%",
            f"{bench['ann_return']*100:.1f}%",
            f"{(strat['ann_return']-bench['ann_return'])*100:+.1f}%",
        ],
        [
            "Annual Volatility",
            f"{strat['ann_vol']*100:.1f}%",
            f"{bench['ann_vol']*100:.1f}%",
