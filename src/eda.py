...
def outlier_days(pct_series: pd.Series, z_thresh: float = 3.0, lookback: int = 21) -> pd.DataFrame:
...
def run_eda(adj_df: pd.DataFrame, log_ret: pd.DataFrame, pct_ret: pd.DataFrame, artifacts_dir: str = "artifacts") -> None:
    """Perform exploratory data analysis."""
    # Plot prices
    fig1 = plt.figure(figsize=(12, 6))
    adj_df.plot(ax=plt.gca(), title="Adjusted Close — TSLA / BND / SPY")
    plt.xlabel("Date"); plt.ylabel("Price")
    fig1.savefig(os.path.join(artifacts_dir, "prices_timeseries.png"), bbox_inches='tight')
    plt.close(fig1)

    # Plot daily % returns
    fig2 = plt.figure(figsize=(12, 6))
    (pct_ret * 100).plot(ax=plt.gca(), alpha=0.8, title="Daily % Returns — TSLA / BND / SPY")
    ...
    plt.xlabel("Date"); plt.ylabel("Volatility (annualized)")
    fig3.savefig(os.path.join(artifacts_dir, "rolling_volatility.png"), bbox_inches='tight')
    plt.close(fig3)
    ...
    pd.DataFrame(risk_rows).to_csv(os.path.join(artifacts_dir, "risk_metrics.csv"), index=False)