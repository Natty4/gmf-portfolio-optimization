...
def fit_arima_on_log_returns(tsla_log_ret_train: pd.Series):
    ...
    model = auto_arima(tsla_log_ret_train.dropna(), start_p=0, start_q=0,
                       max_p=5, max_q=5, d=0, seasonal=False,
                       information_criterion='aic', stepwise=True, suppress_warnings=True,
                       error_action='ignore')
    return model

def arima_predict_prices(model, tsla_log_ret_train: pd.Series, tsla_price_train_last: float, steps: int):
...
def fit_lstm(train_series: pd.Series, val_split: float = 0.1, lookback: int = 60, seed: int = 42):
    """Train LSTM model."""
    ...
    history = model.fit(
        x, y, epochs=30, batch_size=32, validation_split=val_split, callbacks=[es], verbose=0
    )
    return model, scaler

def lstm_recursive_forecast(model, scaler: StandardScaler, train_series: pd.Series, steps: int, lookback: int = 60):
