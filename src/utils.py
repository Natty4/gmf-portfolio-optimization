...
def sharpe_ratio(mean_daily_ret: float, daily_vol: float, rf_daily: float = 0.0) -> float:
...
def chronological_split(df: pd.DataFrame, train_end: str, test_end: str) -> tuple[pd.DataFrame, pd.DataFrame]:
...
def reconstruct_prices_from_log_returns(last_price: float, fc_log_rets: np.ndarray) -> np.ndarray:
...
def residual_bootstrap_intervals(actual_log_ret: pd.Series, pred_log_ret: np.ndarray, n_boot: int = 200, alpha: float = 0.05):
    """Calculate prediction intervals using residual bootstrap."""
    residuals = actual_log_ret[-1000:].dropna().values - 0  # center at 0
    if len(residuals) < 50:
        residuals = np.random.normal(0, np.std(pred_log_ret) if len(pred_log_ret) > 1 else 0.01, size=500)
