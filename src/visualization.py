...
def create_interactive_dashboard(adj_df, forecast_df, backtest_results, artifacts_dir):
    ...
    fig = make_subplots(rows=2, cols=1, subplot_titles=("Price History & Forecast", "Backtest Performance"))
    
    # Price history (last 2 years for clarity)
    recent_data = adj_df['TSLA'].iloc[-504:]  # ~2 years of trading days
    fig.add_trace(go.Scatter(x=recent_data.index, y=recent_data, name='TSLA Historical', line=dict(color='blue')), row=1, col=1)
    
    # Forecast
    fig.add_trace(go.Scatter(x=forecast_df.index, y=forecast_df['Price_FC'], name='Forecast', line=dict(color='red')), row=1, col=1)
    fig.add_trace(go.Scatter(x=forecast_df.index, y=forecast_df['Lower'], name='Lower CI', line=dict(dash='dash', color='gray'), fill=None), row=1, col=1)
    fig.add_trace(go.Scatter(x=forecast_df.index, y=forecast_df['Upper'], name='Upper CI', line=dict(dash='dash', color='gray'), fill='tonexty'), row=1, col=1)
    
    # Backtest comparison
    fig.add_trace(go.Scatter(x=strat_cum.index, y=strat_cum.values, name='Strategy', line=dict(color='green')), row=2, col=1)
    fig.add_trace(go.Scatter(x=bench_cum.index, y=bench_cum.values, name='Benchmark', line=dict(color='orange')), row=2, col=1)
    
    fig.update_layout(height=800, title_text="GMF Investment Analysis Dashboard", showlegend=True)
...
def performance_attribution(weights, returns, artifacts_dir):
    ...
    fig = px.pie(values=asset_contribution.values, names=asset_contribution.index, 
...
def enhanced_risk_analysis(pct_ret, artifacts_dir):
    ...
    for asset in pct_ret.columns:
        fig.add_trace(go.Scatter(x=drawdown.index, y=drawdown[asset], name=f"{asset} Drawdown"))
    ...
    fig = px.imshow(corr_matrix, text_auto=True, aspect="auto", title="Asset Correlation Matrix")
...
def create_model_comparison_chart(arima_metrics, lstm_metrics, artifacts_dir):
    ...
    bars1 = ax.bar(x - width/2, arima_values, width, label='ARIMA', alpha=0.8, color='#1f77b4')
    bars2 = ax.bar(x + width/2, lstm_values, width, label='LSTM', alpha=0.8, color='#ff7f0e')
    ...
    plt.tight_layout()
    plt.savefig(f'{artifacts_dir}/model_performance_comparison.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def create_efficient_frontier_chart(result, artifacts_dir):
    ...
    ax.scatter(max_sharpe_vol, max_sharpe_ret, marker='*', s=300, 
               label=f'Max Sharpe (Return: {max_sharpe_ret:.1%}, Risk: {max_sharpe_vol:.1%})', 
               color='red', edgecolors='black')
    ax.scatter(min_vol_vol, min_vol_ret, marker='D', s=200,
...
def create_backtest_chart(strat_cum, bench_cum, artifacts_dir):
    ...
    ax.plot(strat_cum.index, strat_cum.values, label=f'Strategy ({strat_return:.1f}%)', 
            linewidth=2.5, color='#2ca02c')
    ax.plot(bench_cum.index, bench_cum.values, label=f'Benchmark 60/40 ({bench_return:.1f}%)', 
            linewidth=2.5, color='#d62728', linestyle='--')
    
    # Add outperformance annotation if there's significant data
    if len(strat_cum) > 10:
        mid_point = len(strat_cum) // 2
        ax.annotate(f'Outperformance: +{outperformance:.1f}%', 
                   xy=(strat_cum.index[mid_point], (strat_cum.values[mid_point] + bench_cum.values[mid_point])/2),
    ...
    plt.tight_layout()
    plt.savefig(f'{artifacts_dir}/backtest_comparison.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def create_forecast_chart(historical_prices, forecast_data, artifacts_dir):
    ...
    historical_recent = historical_prices.iloc[-252:] if len(historical_prices) > 252 else historical_prices
    ax.plot(historical_recent.index, historical_recent, label='Historical Prices', 
            linewidth=2, color='#1f77b4')
    
    # Plot forecast if available
    if forecast_data is not None and len(forecast_data) > 0:
        ax.plot(forecast_data.index, forecast_data['Price_FC'], label='Forecast', 
                linewidth=2.5, color='#ff7f0e')
        
        # Plot confidence intervals if available
        if 'Lower' in forecast_data.columns and 'Upper' in forecast_data.columns:
            ax.fill_between(forecast_data.index, forecast_data['Lower'], forecast_data['Upper'], 
                           alpha=0.3, label='95% Confidence Interval', color='#ff7f0e')
...
def create_risk_dashboard(pct_ret, artifacts_dir):
    ...
    volatilities = [risk_data[asset]['ann_vol'] * 100 for asset in assets if asset in risk_data]
    assets_vol = [asset for asset in assets if asset in risk_data]
    
    if volatilities:
        bars = ax1.bar(assets_vol, volatilities, color=colors[:len(assets_vol)], alpha=0.8)
    ...
    sharpe_ratios = [risk_data[asset]['sharpe'] for asset in assets if asset in risk_data]
    assets_sharpe = [asset for asset in assets if asset in risk_data]
    
    if sharpe_ratios:
        bars = ax2.bar(assets_sharpe, sharpe_ratios, color=colors[:len(assets_sharpe)], alpha=0.8)
    ...
    var_95 = [risk_data[asset]['var_95'] * 100 for asset in assets if asset in risk_data]
    ...
    if drawdowns:
        bars = ax4.bar(assets_drawdown, drawdowns, color=colors[:len(assets_drawdown)], alpha=0.8)
    ...
    plt.tight_layout()
    plt.savefig(f'{artifacts_dir}/risk_metrics_dashboard.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def create_allocation_charts(result, artifacts_dir):
    ...
    if 'weights_max_sharpe' in result:
        ...
        wedges1, texts1, autotexts1 = ax1.pie(max_sharpe_weights, labels=max_sharpe_labels, 
                                             autopct='%1.1f%%', colors=colors[:len(max_sharpe_weights)], startangle=90)
    ...
    if 'weights_min_vol' in result:
        ...
        wedges2, texts2, autotexts2 = ax2.pie(min_vol_weights, labels=min_vol_labels, 
                                             autopct='%1.1f%%', colors=colors[:len(min_vol_weights)], startangle=90)
        ax2.set_title('Minimum Volatility Portfolio Allocation\n(Conservative Strategy)')
    ...
    plt.tight_layout()
    plt.savefig(f'{artifacts_dir}/portfolio_allocation.png', dpi=300, bbox_inches='tight')
    plt.close()

def generate_all_visualizations(arima_metrics, lstm_metrics, result, strat_cum, bench_cum, 
                              historical_prices, forecast_data, pct_ret, artifacts_dir):
