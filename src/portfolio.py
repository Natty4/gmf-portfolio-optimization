...
def optimize_portfolio(expected_rets_vec: pd.Series, returns_hist: pd.DataFrame, label: str = "run1"):
    ...
    try:
        if PYPFOPT_AVAILABLE:
            ...
            try:
                ...
                print(f"Max Sharpe optimization failed: {e}. Using Monte Carlo fallback.")
            ...
            for targ_ret in np.linspace(mu.min(), mu.max()*1.5, 80):
                try:
                    ...
                    w_ret.append(ret); w_risk.append(vol); w_sharpe.append(shrp)
                except Exception:
                    continue
            ...
            plt.xlabel("Volatility (σ)"); plt.ylabel("Expected Return (μ)"); plt.title("Efficient Frontier — GMF")
            plt.legend()
            ...
            return {
                ...
                "perf_max_sharpe": {"ret": perf_ms[0], "vol": perf_ms[1], "sharpe": perf_ms[2]},
                "weights_min_vol": cleaned_mv,
                "perf_min_vol": {"ret": perf_mv[0], "vol": perf_mv[1], "sharpe": perf_mv[2]},
                "frontier_image": path
            }
        else:
...
def monte_carlo_optimization(expected_rets_vec: pd.Series, returns_hist: pd.DataFrame, label: str):
    ...
    plt.xlabel("Volatility (σ)"); plt.ylabel("Expected Return (μ)"); plt.title("Efficient Frontier — Monte Carlo")
    plt.legend()
    ...
    return {
        ...
        "perf_max_sharpe": {"ret": float(port_rets[i_ms]), "vol": float(port_vols[i_ms]), "sharpe": float(sharpe[i_ms])},
        "weights_min_vol": dict(zip(returns_hist.columns, weights[i_mv].round(4))),
        "perf_min_vol": {"ret": float(port_rets[i_mv]), "vol": float(port_vols[i_mv]), "sharpe": float(sharpe[i_mv])},
        "frontier_image": path
    }

def validate_expected_returns(expected_rets: pd.Series, max_annual_return: float = 0.5) -> pd.Series:
    ...
    for asset in validated.index:
        if np.isnan(validated[asset]) or np.isinf(validated[asset]):
            print(f"Warning: {asset} has invalid expected return {validated[asset]}. Using default.")
            if asset == 'TSLA':
            ...
            print(f"Warning: Capping {asset} expected return from {validated[asset]:.2f} to {max_annual_return:.2f}")
